...
def calculate_forward_probabilities(observations: np.ndarray, fair_dice: dice.Dice, loaded_dice: dice.Dice,
    ...
    alpha_probabilities[0] /= np.sum(alpha_probabilities[0])
...
def calculate_backward_probabilities(observations: np.ndarray, fair_dice: dice.Dice, loaded_dice: dice.Dice,
    ...
    beta_probabilities[observations_rows-1] = np.array([.5, .5])
...
def baum_welch(multiple_observations: List[np.ndarray], T=None, emMat=None, pi0=None, epochs: int = 50) -> Tuple[dice.Dice, dice.Dice, np.ndarray, np.ndarray]:
    ...
    first_dice_probabilities=emMat[0,:]
    ...
    second_dice_probabilities=emMat[1,:]
